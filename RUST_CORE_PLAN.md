...
- **Fase 1 — Contrato de serviço + 1º plugin (App Store Connect).** `service::{provider, kind, registry}` (`Provider`/`ServiceKind`/`Capability`/`CredentialField`); `auth::es256`; `providers/appstore` com `validate` + `fetch_apps` (`GET /v1/apps`, paginação `links.next`); facade `connect`/`credential_schema`/`available_services`. Plugar a validação/listagem da Apple no app via o core (strangler).
